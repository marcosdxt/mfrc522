...
//const unsigned char UNUSED_PIN = UINT8_MAX;
...
extern void RC522TARGET_spi_cs_0(unsigned char state);
/*****************************************************************************************************
...
/*****************************************************************************************************
*      Description: Writes a unsigned char to the specified register in the MFRC522 chip.
*       The interface is described in the datasheet section 8.1.2.
*       (PCD_Register) endereço do registrador
*       (unsigned char) valor
*****************************************************************************************************/
void PCD_WriteRegister(	PCD_Register reg,unsigned char value){
  
   RC522TARGET_spi_cs_0(0);         
   
   spi_write_single(reg);
   spi_write_single(value);
   
   RC522TARGET_spi_cs_0(1);                                           
...
*       (unsigned char) valor
*****************************************************************************************************/
void PCD_WriteRegisterBlock(	PCD_Register reg,unsigned char count,unsigned char *values){
                                                                  
   RC522TARGET_spi_cs_0(0);    
   
   spi_write_single(reg);
   for(unsigned char i=0;i<count;i++)
     spi_write_single(*values++);
...
unsigned char PCD_ReadRegister(	PCD_Register reg) {
  unsigned char value;
   ...
   spi_write_single(reg|0x80);
   value = spi_read_single(0);
...
* The interface is described in the datasheet section 8.1.2.
*****************************************************************************************************/
void PCD_ReadRegisterBlock(PCD_Register reg,unsigned char count,unsigned char *values,unsigned char rxAlign){
  unsigned char address = 0x80 | reg;	
  ...
  if (rxAlign) {
    ...
    unsigned char value = spi_read_single(address);
    values[0] = (values[0] & ~mask) | (value & mask);
    index++;
  }

  while (index < count) {
    values[index] = spi_read_single(address); //SPI.transfer(address);	// Read value and tell that we want to read the same address again.
    index++;
  }
  values[index] = spi_read_single(0);//SPI.transfer(0);			// Read the final unsigned char. Send 0 to stop reading.
...
StatusCode PCD_CalculateCRC(unsigned char *data,unsigned char length,unsigned char *result){
  ...
  PCD_WriteRegisterBlock(FIFODataReg, length, data);	// Write data to the FIFO
  ...
  for (unsigned short int i = 5000; i > 0; i--) {
...
******************************************************************************************************/
unsigned char PCD_PerformSelfTest(void){
	...
	PCD_WriteRegisterBlock(FIFODataReg, 25, ZEROES);	// write 25 unsigned chars of 00h to FIFO
	...
	PCD_ReadRegisterBlock(FIFODataReg, 64, result, 0);
...
void PCD_SoftPowerUp(void){
	...
	unsigned int timeout = 500;//(unsigned int)millis() + 500;// create timer for timeout (just in case) 
	
	while(timeout--){ // set timeout to 500 ms 
...
StatusCode PCD_CommunicateWithPICC(	unsigned char command,		///< The command to execute. One of the PCD_Command enums.
	...
	PCD_WriteRegister(FIFOLevelReg, 0x80);				// FlushBuffer = 1, FIFO initialization
	PCD_WriteRegisterBlock(FIFODataReg, sendLen, sendData);	// Write sendData to the FIFO
	...
	if (backData && backLen) {
		...
		*backLen = n;											// Number of unsigned chars returned
		PCD_ReadRegisterBlock(FIFODataReg, n, backData, rxAlign);	// Get received data from FIFO
...
StatusCode PICC_Select(	Uid *uid,			///< Pointer to Uid struct. Normally output, but can also be used to supply a known UID.
	...
	while (!uidComplete) {
		...
		unsigned char charsToCopy = currentLevelKnownBits / 8 + (currentLevelKnownBits % 8 ? 1 : 0); // The number of unsigned chars needed to represent the known bits for this level.
		if (charsToCopy) {
			unsigned char maxchars = useCascadeTag ? 3 : 4; // Max 4 unsigned chars in each Cascade Level. Only 3 left if we use the Cascade Tag
			if (charsToCopy > maxchars) {
			   charsToCopy = maxchars;
			}
			for (count = 0; count < charsToCopy; count++) {
		...
		charsToCopy		= (buffer[2] == PICC_CMD_CT) ? 3 : 4;
		for (count = 0; count < charsToCopy; count++) {
...
const char *PICC_GetTypeName(PICC_Type piccType	///< One of the PICC_Type enums.
	...
	switch (piccType) {
		...
		case PICC_TYPE_MIFARE_MINI:		return "MIFARE Mini, 320 unsigned chars";
...
void PICC_DumpDetailsToSerial(Uid *uid) {
  /*
	...
	for (unsigned char i = 0; i < uid->size; i++) {
		if(uid->uidunsigned char[i] < 0x10)
			Serial.print(F(" 0"));
		else
			Serial.print(F(" "));
...
void PICC_DumpMifareClassicToSerial(	Uid *uid,			///< Pointer to Uid struct returned from a successful PICC_Select().
	...
	if (no_of_sectors) {
		...
		for (unsigned char i = no_of_sectors - 1; i >= 0; i--) {
...
void PICC_DumpMifareClassicSectorToSerial(Uid *uid,
	...
	for (unsigned char blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
...
//			  PICC_WakeupA(atqa_answer, &atqa_size);
	...
	unsigned char bcc = 0;
	for (unsigned char i = 0; i < uidSize; i++) {
...
unsigned char PICC_IsNewCardPresent() {
	...
	return result;//(result == STATUS_OK || result == STATUS_COLLISION);
